...
### 修复
- **建议项在 zsh 5.8 和 5.8.1 上失去了颜色，原因正是给它起名字的那个标记。**
  `region_highlight` 的条目会按属性位重新渲染，所以一个 highlighter 想再认出自己写的那条，
  就必须在文本里留下点什么——我们写的是 `… fg=8 memo=zsh-smart-complete:suggestion`。
  但 `memo=` 只有 zsh 5.9 及以后（`Src/Zle/zle_refresh.c`）才解析；在 5.8 上，
  `Src/prompt.c: match_highlight()` 会在把颜色存下来*之前*走到 `else if (*teststr) break;`，
  于是颜色之后任何不是逗号的 token 都会把剩下的扫描整个丢掉。条目最终带着**零**属性被存下——
  实测为 `rh=1 12 none`，而该会话自己的 `terminfo[colors]` 是 256，这也排除了前两版修复所追的
  terminfo 假设。现在只有当运行中的 zsh 能携带这个标记时才写它；在更早的构建上，模块靠“记住自己写过什么”
  来认自己的条目，并记下它的起点——因为补全列表重绘会把区间裁剪回 `BUFFER` 末尾，被记住的文本就不再匹配了。
  README 承诺的性质在两条分支上都成立：最多一条，且只移除那一条。
- **版本矩阵的三个 cell 变红，三个原因都属于宿主而不是代码。** 共享的桩件农场把 `opkg` 放进了
  `PATH`，于是 `install.sh` 在每次 Linux 运行时都移交给 `install-entware.sh`——被测试的脚本被自己的
  桩件换掉了；那个看起来像 Linux 专属安装器 bug 的“写了一半的 `.zshrc`”，其实是**另一个**安装器的文件，
  它按设计就不带受管标记。`tools/check-module-globals.sh` 用 `{n,}` 区间表达式提取文档里的键列表，
  而 Debian/Ubuntu 镜像的 `awk` 是 mawk，在 POSIX 模式之外会忽略区间：它一个键都没提到，于是每条
  dead-key 断言都因为“什么都没检查”而通过。两个安装器套件需要 `python3` 和 `ps`，最小镜像不提供。
  这些 cell 继续保持阻塞——上面那个颜色 bug 正是只有它们能发现的问题。

### 变更
- **一个红灯 job 现在会公布它掌握的每一个失败。** 从 runner 外面读 job 日志需要认证，
  而它 check run 的注解不需要。三个 job 各自抄了一份打包逻辑，也各自丢掉过自己已经查到的东西：
  对宽 grep 结果做 `tail -n 5` 只公布了七个失败断言里的一个（两次运行返回字节完全相同，因为没人读得懂）；
  端到端步骤的 `tail -n 20` 槽位被引出的屏幕文本占满，把失败本身挤掉了；而 release gate 完全没有这套东西——
  它对 v2.4.0 那次失败只说了一句 `Process completed with exit code 1`。现在唯一的实现是
  `tools/annotate.sh`：它按*条目*边界把一个长字段折成多条注解（按列切断会把两条断言的名字交错在一起，两边都读不出来），
  并且即使在一片绿的时候也公布每个套件自己的 `name:passed/failed` 对——这让 README 里引用的断言数量可以在
  做出该测量的那次运行里被核对。它永远 exit 0：一个能把构建判红的诊断工具，就是第二套构建系统。
- v2.3.0 那条记录里写矩阵镜像打包的是 zsh 5.7.1 / 5.8.1 / 5.9。每个 cell 自己的 `zsh --version`
  报的是 **5.8** / 5.8.1 / 5.9，所以 `ubuntu:focal` 那个数字是错的；现在五种语言都改成 5.8，
  workflow 里也写明了这些数字的出处。

### 新增
- `tests/test-annotate.sh`：基于桩件日志的 39 条断言，其中包括“没有任何注解宽过脚本自己声明的预算”——
  这条量的是工具真正吐出的字节，因为拿一个重新实现的打包逻辑去断言，无论脚本怎么折都会通过。
  `tests/test-display.zsh` 增加了一个场景（55 → 74 条断言），它会强制走无标记分支，检查条目不会每次按键叠一条、
  被裁剪后的残留仍能被认出并移除，以及版本门控把 `5.10` 读成比 `5.9` 更新而不是当成 `5.1`。
