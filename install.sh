...
# Interactive .bak.* cleanup: list all backup artifacts, let user choose which to remove.
# Cascaded backups (multiple .bak. in name) are always offered for removal.
# Stale backups (older than newest per stem) are also listed for user choice.
# The user may skip any or all items.  Zero-auto-delete policy: we never remove
# without explicit user confirmation.
_cleanup_old_baks() {
    local -a item_paths=() item_labels=() item_types=()
    local base f stem cand cbn cts newest_ts newest_path
    # Collect all .bak.* config files under ZDOTDIR and $HOME/.config
    for base in "$ZDOTDIR"/.zshrc.bak.* \
                "$ZDOTDIR"/.p10k.zsh.bak.* \
                "$ZDOTDIR"/.config/starship.toml.bak.* \
        ...
        if [[ "$f" == *.bak.*.bak.* ]]; then
            ...
            stem="${f%.bak.*}"
            cts="${f##*.bak.}"
            # Find the newest timestamp for this stem to flag stale backups
            newest_ts=0; newest_path=""
            for cand in "$ZDOTDIR"/"${stem}".bak.* "$ZDOTDIR"/.config/"${stem}".bak.* \
                        "$HOME"/.config/"${stem}".bak.*; do
                [[ -e "$cand" ]] || continue
                cbn="$(basename "$cand")"
                [[ "$cbn" == *.bak.*.bak.* ]] && continue
                cts="${cbn##*.bak.}"
                if (( cts > newest_ts )); then
                    newest_ts="$cts"; newest_path="$cand"
                fi
            done
            if [[ "$base" == "$newest_path" ]]; then
                item_labels+=("$f (newest)")
                item_types+=("newest")
            else
                item_labels+=("$f (stale)")
                item_types+=("stale")
...
_cleanup_conflict_residues() {
    ...
    for pattern in "*autocomplete*" "*autosuggestions*"; do
        for pdir in "$ZINIT_PLUGINS_DIR"/"$pattern"; do
            [[ -d "$pdir" ]] || continue
            residues+=("$pdir")
            residue_labels+=("conflict plugin: $(basename "$pdir")")
        done
    done
    (( ${#residues[@]} )) || { info "No conflict-plugin residue found"; return 0; }
    echo
    info "Found ${#residues[@]} conflict-plugin residue item(s):"
    local i=1
    for (( i=1; i<=${#residue_labels[@]}; i++ )); do
        printf "  %2d) %s\n" "$i" "${residue_labels[$((i-1))]}"
    done
    echo -n "$(msg prompt.residue_clean)"
    local confirm=""; read -r confirm || true
    case "$confirm" in
        y|Y)
            # Remove all without individual prompts
            for (( i=1; i<=${#residues[@]}; i++ )); do
                rm -rf "${residues[$((i-1))]}" && success "Removed: ${residue_labels[$((i-1))]}"
            done ;;
        *)
            # Show individual items; default yes for conflict dirs
            for (( i=1; i<=${#residues[@]}; i++ )); do
                local label="${residue_labels[$((i-1))]}"
                local default_yes=0
                [[ "$label" == *"conflict plugin"* ]] && default_yes=1
                if prompt_yes "$(msg prompt.bak_remove "$label")" "$default_yes"; then
                    rm -rf "${residues[$((i-1))]}" && success "Removed: $label"
                fi
            done ;;
    esac
    info "Conflict residue cleanup done."
}
_cleanup_conflict_residues
