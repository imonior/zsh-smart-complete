...
- **CI 現在是三個 job，依各自能證明什麼來劃分。** `test` 是門檻（macOS + Linux）。`zsh-versions` 在 `ubuntu:focal` / `ubuntu:jammy` / `debian:bookworm` 裡跑同一套測試，也就是打包的 zsh 5.7.1 / 5.8.1 / 5.9——因為本倉庫大量程式碼把結論寫成「在 zsh 5.9 上實測」，卻只在每個系統的一個建置上驗證過。`e2e-advisory` 以**非阻擋**方式跑 tmux 端對端集：它是唯一能看見渲染後螢幕的測試集，也是唯一用真實終端 + 牆鐘等待來驅動的，所以必須先測出它的抖動率，才有資格把一次好改動判紅。release 工作流新增了一個 `release` 所依賴的 `gate` job——測試必須全綠、`VERSION` 必須與 tag 一致、CHANGELOG 必須有對應章節；先前對著人們真正安裝的 tag 直接發佈是無條件的。
