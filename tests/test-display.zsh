...
# Our single entry, plus how many of the entries on screen are ours.
_own_rh=""
_own_n=0
_scan_rh() {
    local r
    ...
    for r in "${region_highlight[@]}"; do
        [[ "$r" == *"memo=${_SMART_RH_MARKER}"* ]] && { ((_own_n++)); _own_rh="$r"; }
    done
}
_foreign_n() {
    local r n=0
    for r in "${region_highlight[@]}"; do
        [[ "$r" == *"memo=${_SMART_RH_MARKER}"* ]] || (( n++ ))
    done
    print -r -- "$n"
}
...
assert_eq "exact entry text"         "$_own_rh"   "5 10 fg=8  memo=${_SMART_RH_MARKER}"
...
assert_eq "re-put moved the range"   "$_own_rh"   "2 4 fg=8  memo=${_SMART_RH_MARKER}"
...
# BUFFER, i.e. zero-length, i.e. no colour.
_reset_screen
region_highlight=("5 5 fg=8  memo=${_SMART_RH_MARKER}")
assert_rc "no ghost -> no-op" 0 _smart_display_reassert_rh
_scan_rh
assert_eq "untouched while POSTDISPLAY is empty" "$_own_rh" "5 5 fg=8  memo=${_SMART_RH_MARKER}"
BUFFER="git s"; POSTDISPLAY="tatus"
_smart_display_reassert_rh
_scan_rh
assert_eq "re-clipped range restored" "$_own_rh" "5 10 fg=8  memo=${_SMART_RH_MARKER}"
assert_eq "still exactly one of ours" "$_own_n"  "1"
region_highlight=("0 3 bold fg=yellow" "5 5 fg=8  memo=${_SMART_RH_MARKER}")
BUFFER="ls"; POSTDISPLAY=" -la"
_smart_display_reassert_rh
_scan_rh
assert_eq "reassert from an empty BUFFER" "$_own_rh" "2 6 fg=8  memo=${_SMART_RH_MARKER}"
...
_scan_rh
assert_eq "ghost is coloured" "$_own_rh" "5 10 fg=8  memo=${_SMART_RH_MARKER}"
...
region_highlight=("3 13 fg=8  memo=${_SMART_RH_MARKER}" "0 3 bold fg=yellow")
