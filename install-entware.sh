...
# `-u`, for the same reason as install.sh (see the note there). The sandbox
# suite can only drive this script to its `opkg not found` guard on a normal
# machine, so what is actually proven is the shared preamble: the strict mode,
# the i18n table and the marker constants.
set -euo pipefail
IFS=$'\n\t'
...
select_mirror() {
    ...
    if [[ "${NONINTERACTIVE:-0}" == "1" ]]; then
        ...
        if [[ -n "$best" ]]; then
            ...
            info "$(msg mirror.auto_fastest "${MIRROR_LABELS[$best]}" "${MIRROR_TIMES[$best]:-999}" "${GH_MIRROR_TYPE}")"
        else
    ...
    for (( ii=0; ii<n; ii++ )); do
        ...
        printf "  %2d) %s%s  [%ss]\n" "$d" "${MIRROR_LABELS[$i]}" "$mark" "${MIRROR_TIMES[$i]:-999}"
        d=$((d+1))
    done
    printf "  %2d) %s\n" "$d" "$(msg mirror.manual)"
...
TOML
        ...
        if run_with_mirror_dl '_run_remote_script https://starship.rs/install.sh -y' 2>/dev/null; then
...
# The loader-block markers (values defined in the shared core above, because an
# uninstall needs them before this point is ever reached). This installer does
# not put those markers around its own integration block (it appends the loader
# once and then leaves it alone — see the `grep -q "zsh-smart-complete"` guard
# below Phase 4), but `_upsert_options_block` compares against these names.
#
...
# it does so as a sequence of writes. Arm the snapshot/rollback guard around
